...
def get_oai_metadata():
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "configs", "mysql.yml")) as mysql_config_file:
    ...
    if error is not None:
        ...
        # NOTE - Result will be a list of dicts
        root_tag = "OAI-PMH"
        ...
        if valid_params["verb"] in ["ListRecords", "ListIdentifiers", "GetRecord"]:
            ...
            root_attrs["{%s}schemaLocation" % xsi] = "http://www.openarchives.org/OAI/2.0/ http://www.openarchives.org/OAI/2.0/OAI-PMH.xsd"
        else:
        ...
        # First create document and start the root element
        root_element = Element("{%s}%s" % (xmlns, root_tag), root_attrs, nsmap=namespace_map)
